        ...
        if (data.success) {
            ...
            if (li) {
                if (data.eliminado) {
                    ...
                    li.querySelector('small').textContent = `${data.cantidad} x $${data.precio}`;
                    li.querySelector('span.text-body-secondary').textContent = `$${data.subtotal}`;
                }
            }

            const totalGeneral = document.querySelector('li.total span.total-valor');
            if (totalGeneral) {
...
function eliminarProducto(id) {
    ...
    .then(data => {
        if (data.success) {
            ...
            if (item) {
                // Obtener y restar el subtotal del producto eliminado
                const subtotalTexto = item.querySelector('.subtotal-texto');
                const subtotal = parseFloat(subtotalTexto.textContent.replace('$', ''));
                
                const totalGeneral = document.getElementById('total-general');
                if (totalGeneral) {
                    const totalActual = parseFloat(totalGeneral.textContent);
                    totalGeneral.textContent = (totalActual - subtotal).toFixed(2);
                }
            ...
            const carrito = document.querySelectorAll('li[data-id]');
            if (carrito.length === 0) {
