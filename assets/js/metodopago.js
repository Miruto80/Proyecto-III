...
$('#btnAbrirRegistrar').on('click', function () {
  ...
  $('#modalTitle').text('Registrar Metodo de Pago');
...
function validarFormulario(formId) {
    ...
    if (formId === 'formRegistrar') {
        ...
        if (validarkeyup(/^.{3,70}$/, $("#descripcion"), $("#sdescripcion"), "La descripcion debe tener entre 3 y 100 caracteres") === 0) {
            valido = false;
        }
        ...
        if (validarkeyup(/^.{3,70}$/, $("#descripcion_modificar"), $("#sdescripcion_modificar"), "La descripcion debe tener entre 3 y 100 caracteres") === 0) {
            valido = false;
        }
    }
...
function abrirModalModificar(id_metodopago) {
    // Consultar datos del metodopago por ID
    const datos = new FormData();
    datos.append('id_metodopago', id_metodopago);
    datos.append('consultar_metodopago', 'consultar_metodopago');
    ...
    .then(data => {
        document.getElementById('id_metodopago_modificar').value = data.id_metodopago;
    ...
    .catch(error => {
        muestraMensaje("error", 2000, "Error", "ERROR al cargar los datos del metodo de pago: " + error);
    });
}
...
function eliminarMetodoPago(id_metodopago) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            const datos = new FormData();
            datos.append('id_proveedor', id_metodopago);
    ...
    .then(data => {
        if (data.accion === 'incluir') {
            if (data.respuesta === 1) {
                ...
                muestraMensaje("success", 1500, "Se ha registrado con éxito", "El Metodo de pago se ha registrado exitosamente");
                setTimeout(() => location.reload(), 1500);
            } else {
                muestraMensaje("error", 2000, "ERROR", "ERROR al registrar el Metodo de pago");
            }
        } else if (data.accion === 'actualizar') {
            if (data.respuesta === 1) {
                muestraMensaje("success", 1500, "Se ha Modificado con éxito", "El Metodo de pago se ha actualizado exitosamente");
                setTimeout(() => location.reload(), 1500);
            } else {
                muestraMensaje("error", 2000, "ERROR", "ERROR al modificar el Metodo de pago");
            }
        } else if (data.accion === 'eliminar') {
            if (data.respuesta === 1) {
                muestraMensaje("success", 1500, "Se ha eliminado con éxito", "El Metodo de pago se ha eliminado correctamente");
                setTimeout(() => location.reload(), 1500);
            } else {
                muestraMensaje("error", 2000, "ERROR", "ERROR al eliminar el Metodo de pago");
            }
        }
