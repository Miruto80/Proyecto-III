...
function validarDireccion(input) {
    ...
    if (!valido) mostrarError(input, "Mínimo 10 caracteres.");
    else limpiarError(input);
    return valido;
}
...
function validarFormularioPedido() {
    ...
    const entrega = document.getElementById("metodoentrega");
    ...
    const validaciones = [
        ...
        validarSelect(entrega, "Seleccione un método de entrega válido."),
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById('btn-guardar-pedido').addEventListener('click', async (e) => {
        ...
        try {
            const resPedido = await fetch('controlador/verpedidoweb.php', { method: 'POST', body: formData });
            ...
            muestraMensaje("success", 2000, "Pedido registrado", "Redirigiendo...");
    ...
    dir.addEventListener("input", () => validarDireccion(dir));
    document.querySelectorAll('select').forEach(sel =>
        sel.addEventListener("change", () => validarFormularioPedido())
    );
});
