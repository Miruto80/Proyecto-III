...
const TEST_CASE_EXTERNAL_ID = '1-35'; // <-- debe ser algo como PROY-34, ABC-34, LM-34
...
const deleteBtn = await findFirstAvailable(driver, [
    By.css("table tbody tr:first-child button.btn-eliminar"),
    By.css("table tbody tr:first-child button.btn-danger"),
    By.xpath("//button[contains(@class,'btn-eliminar')]"),
    By.xpath("//button[i[contains(@class,'fa-x')]]")
]);

if (!deleteBtn) throw new Error("No se encontró botón Rechazar (X)");

await driver.executeScript("arguments[0].scrollIntoView(true);", deleteBtn);
await driver.sleep(300);
await clickForce(driver, deleteBtn);

// ===== SWEETALERT =====
console.log("➤ Esperando SweetAlert de eliminación...");

const swalDeleteConfirm = await findFirstAvailable(driver, [
    By.css(".swal2-confirm"),
    By.xpath("//button[contains(text(),'Sí')]"),
    By.xpath("//button[contains(text(),'eliminar')]"),
    By.xpath("//button[contains(@class,'swal2-confirm')]")
]);

if (!swalDeleteConfirm) throw new Error("No apareció SweetAlert para eliminar");

await driver.wait(until.elementIsVisible(swalDeleteConfirm), 8000);
await driver.wait(until.elementIsEnabled(swalDeleteConfirm), 8000);

console.log("➤ Confirmando eliminación...");
await clickForce(driver, swalDeleteConfirm);

console.log("✅ Pedido eliminado por SweetAlert");

// marcar prueba como exitosa
status = 'p';
notes = 'Pedido rechazado correctamente.';
