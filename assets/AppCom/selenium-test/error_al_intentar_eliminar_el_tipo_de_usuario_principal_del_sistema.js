...
const TEST_CASE_EXTERNAL_ID = 'R-1-30'; // 🔹 Código de prueba 1-30
const TEST_PLAN_ID = 2;
const BUILD_NAME = 'v.1';

// === TEST AUTOMATIZADO: Error al eliminar el tipo usuario principal ===
async function runTest() {
  ...
  try {
    ...
    // === Buscar botón de eliminar del tipo usuario principal (id = 2) ===
    console.log('🔎 Buscando el botón de eliminar del tipo usuario principal...');
    const botones = await driver.findElements(By.css('.eliminar'));
    let botonAdministrador = null;

    for (const btn of botones) {
      const id = await btn.getAttribute('value');
      if (id === '2') {
    ...
    if (!botonAdministrador) throw new Error('No se encontró el botón para eliminar el tipo usuario principal (ID 2).');

    // === Intentar eliminar el tipo usuario principal ===
    console.log('🗑️ Intentando eliminar el tipo usuario principal...');
    ...
    if (
      ...
      alertaTexto.includes('no puede eliminarse') ||
      ...
      status = 'p';
      notes = 'El sistema impidió correctamente la eliminación del rol principal (Administrador).';
    } else {
