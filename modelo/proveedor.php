...
class proveedor extends Conexion {
    ...
    private function ejecutarRegistro(array $d): array {
        ...
        try {
            ...
            if (empty($d['nombre']) || empty($d['numero_documento']) || empty($d['tipo_documento'])) {
                throw new Exception("Los datos del proveedor no pueden estar incompletos.");
            }
    ...
    private function ejecutarActualizacion(array $d): array {
        ...
        try {
            ...
            $sqlCheck = "SELECT COUNT(*) FROM proveedor WHERE id_proveedor = :id_proveedor";
            $stmtCheck = $conex->prepare($sqlCheck);
            $stmtCheck->execute(['id_proveedor' => $d['id_proveedor']]);
            $existe = $stmtCheck->fetchColumn();
            
            if ($existe == 0) {
                throw new Exception("El proveedor con ID {$d['id_proveedor']} no existe.");
            }
